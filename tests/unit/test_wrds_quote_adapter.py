...
def test_normalize_wrds_quotes_maps_fixture_into_quote_schema() -> None:
    ...
    assert normalized.loc[0, SOURCE] == "wrds_taq_cqm"
    assert normalized.loc[0, SYMBOL] == "AAPL"
    assert normalized.loc[0, QUOTE_EXCHANGE] == "Q"
    assert normalized.loc[0, BID_EXCHANGE] == "Q"
    assert normalized.loc[1, ASK_EXCHANGE] == "N"
    assert normalized.loc[0, QUOTE_SEQUENCE_NUMBER] == 101
